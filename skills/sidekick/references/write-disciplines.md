...
> Is this **log**, **brain**, or **output/database**?
...
| A database **structure** change (table/column) | `data.sqlite` | **Confirm in plain language.** |
| A database **record** that fits the existing schema | `data.sqlite` | **None — insert freely.** |
...
- **Database structure is confirmed, records are free** because the user
  ...
  every schema change keeps the data model clean.
...
- Database: *"You're now tracking deadlines per task. Shall I add a
